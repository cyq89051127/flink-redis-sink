...
public class HSetMapper extends RowRedisMapper {
    ...
    public HSetMapper(Map<String,String> props, RedisTableSchema redisTableSchema) {
    ...
    public Map<String,String> getValueAsMap(Tuple2<Boolean, Row> data) {
        ...
        if (data.f0){
            ...
            for (int i = 0; i < redisTableSchema.getFieldNames().length; i++) {
                ...
                if (fieldValue == null){
                    // We will overrite all the values, for null field, an empty string is a must.
                    value.put(redisTableSchema.getFieldNames()[i],"");
                }else{
